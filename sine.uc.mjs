...
const Sine = {
        ...
        for (const key of ids) {
            ...
            if (modData.hasOwnProperty("preferences")) {
                ...
                for (const pref of modPrefs) {
                    console.log(pref, modPrefs, modData, modData["name"], modData["id"]);
            ...
            remove.addEventListener("click", async () => {
                if (window.confirm("Are you sure you want to remove this mod?")) {
                    ...
                    if (modData.hasOwnProperty("js")) {
                        await IOUtils.remove(PathUtils.join(PathUtils.join(this.chromeDir, "JS"), `${modData["id"]}.uc.js`));
    ...
    async createThemeJSON(repo, theme={}, mainProcess=false) {
        ...
        const notNull = (data) => {
            console.log(data);
            return typeof data === "object" || (typeof data === "string" && data.toLowerCase() !== "404: not found");
        }
    ...
    async initMarketplace() {
        ...
        const settingPrefs = [
            ...
            {
                "type": "checkbox",
                "property": "sine.is-cool",
                "defaultValue": true,
                "label": "Enable Sine branding."
            },
            {
                "type": "text",
                "label": "**Updates**",
...
switch (document.location.pathname) {
        ...
        window.addEventListener("load", async () => {
            if (document.readyState === "complete") {
                ...
                const listenerFunc = async () => {
                    console.log("LISTENER TRIGGERED", !UC_API.Prefs.get("sine.no-internet")["value"]);
                    if (!UC_API.Prefs.get("sine.no-internet")["value"])
    ...
    case "/content/browser.xhtml":
        ...
        UC_API.Prefs.set("sine.fetch-url", "none");
        console.log('Sine: Fetch URL set to none.');
        ...
        if (Sine.modGitHubs) {
            console.log("Sine mods found in shared storage.");
            await UC_API.SharedStorage.widgetCallbacks.set("transfer", JSON.stringify(Sine.modGitHubs));
            console.log("Sine mods transfered to shared storage.");
            UC_API.Prefs.set("sine.no-internet", false);
        } else {
            UC_API.Prefs.set("sine.no-internet", true);
            console.log("Sine mods not found in shared storage.");
        }
