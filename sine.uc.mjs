...
const Sine = {
    ...
    async toggleTheme(themeData, remove) {
        if (themeData["js"]) var jsPath = PathUtils.join(this.chromeDir, "JS");
        if (remove) {
            await this.manager.disableTheme(themeData["id"]);
            if (themeData.hasOwnProperty("js")) {
                for (const file of themeData["editable-files"]["js"]["contents"]) {
                    await IOUtils.writeUTF8(PathUtils.join(jsPath, file.replace(/[a-z]+\.js$/, "db")), await IOUtils.readUTF8(jsPath));
                    await IOUtils.remove(PathUtils.join(jsPath, file), { ignoreAbsent: true });
                }
                alert("A mod utilizing JS has been disabled. For usage of it to be fully halted, restart your browser.");
            }
        } else {
            await this.manager.enableTheme(themeData["id"]);
            if (themeData.hasOwnProperty("js")) {
                for (const file of themeData["editable-files"]["js"]["contents"]) {
                    await IOUtils.writeUTF8(PathUtils.join(jsPath, file), await IOUtils.readUTF8(jsPath.replace(/[a-z]+\.js$/, "db")));
                    await IOUtils.remove(PathUtils.join(jsPath, file.replace(/[a-z]+\.js$/, "db")), { ignoreAbsent: true });
                }
                alert("A mod utilizing JS has been enabled. For usage of it to be fully restored, restart your browser.");
            }
        }
            ...
            remove.addEventListener("click", async () => {
                if (window.confirm("Are you sure you want to remove this mod?")) {
                    ...
                    if (modData.hasOwnProperty("js")) {
                        for (const file of modData["editable-files"]["js"]["contents"]) {
    ...
    async handleJS(newThemeData) {
        ...
        if (UC_API.Prefs.get("sine.allow-unsafe-js")["value"] &&
           (typeof newThemeData["js"] === "string" || typeof newThemeData["js"] === "array")) {
            ...
            const newFiles = Object.values(await this.fetch(dirLink).catch(err => console.warn(err)));
    ...
    async installMod(repo) {
        ...
        if (newThemeData) {
            ...
            } if (newThemeData.hasOwnProperty("js")) newThemeData["editable-files"] = this.handleJS(newThemeData);
