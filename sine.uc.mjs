...
if (!UC_API.Prefs.get("sine.is-cosine").exists()) UC_API.Prefs.set("sine.is-cosine", true);
if (!UC_API.Prefs.get("sine.mods.disable-all").exists()) UC_API.Prefs.set("sine.mods.disable-all", false);
...
const Sine = {
    ...
    utils: {
        ...
        get chromeFile() {
            return PathUtils.join(Sine.utils.modsDir, "chrome.css");
        },

        get contentFile() {
            return PathUtils.join(Sine.utils.modsDir, "content.css");
        },
    ...
    manager: {
        async rebuildStylesheets() {
            ...
            return { chrome: chromeData, content: contentData };
        },

        async rebuildMods() {
            console.log("[Sine]: Rebuilding styles.");
            const styles = await this.rebuildStylesheets();
    ...
    initDev() {
        if (UC_API.Prefs.get("sine.enable-dev").value) {
            ...
            palette.className = "sineCommandPalette";
            palette.setAttribute("hidden", "");

            const contentDiv = document.createElement("div");
            contentDiv.className = "sineCommandInput";
            contentDiv.setAttribute("hidden", "");
            palette.appendChild(contentDiv);

            const searchDiv = document.createElement("div");
            searchDiv.className = "sineCommandSearch";
    ...
    parsePrefs(pref) {
        ...
        if ((pref.type === "string" || pref.type === "dropdown") && pref.hasOwnProperty("label")) {
            ...
            hboxLabel.className = "sineItemPreferenceLabel";
    ...
    async loadMods() {
        ...
        if (!UC_API.Prefs.get("sine.mods.disable-all").value) {
            ...
            for (const key of ids) {
                ...
                item.className = "sineItem";

                // Create new content.
                const content = document.createElement("vbox");
                content.className = "sineItemContent";
                // Create new header
                const header = document.createElement("hbox");
                header.id = "sineItemContentHeader";
                header.innerHTML = `
                    <label>
                ...
                toggle.className = "sineItemPreferenceToggle";
                ...
                description.className = "description-deemphasized sineItemDescription";
                ...
                actions.className = "sineItemActions";

                // Create new dialog.
                const dialog = document.createElement("dialog");
                dialog.className = "sineItemPreferenceDialog";

                if (modData.preferences) {
                    // Create new top bar.
                    const topbar = document.createElement("div");
                    topbar.className = "sineItemPreferenceDialogTopBar";
                    topbar.innerHTML = `<h3 class="sineItemTitle">${modData.name} (v${modData.version})</h3>`;
                    ...
                    prefs.className = "sineItemPreferenceDialogContent";
                    ...
                    settings.className = "sineItemConfigureButton";
                    settings.innerHTML = this.settingsSVG;
                    settings.title = "Open settings";
                    if (!modData.enabled) toggle.addEventListener("toggle", loadPrefs);
                ...
                remove.className = "sineItemUninstallButton";
                remove.dataset.l10nId = "sine-item-remove-button";
                ...
                document.querySelector("#sineModsList").appendChild(item);
            }
        }
    },
            ...
            (async () => {
                ...
                newItemDescription.className = "sineMarketplaceItemDescription";
                    ...
                    if (data.readme) {
                        ...
                        content.className = "sineItemPreferenceDialogContent";
    ...
    async initSine() {
        const createSineGroup = (group) => {
            group.dataset.category = "paneSineMods";
            if (!this.sineIsActive) group.setAttribute("hidden", "true");
            group.className = "subcategory";
        };
        ...
        const title = document.createElement("hbox");
        title.id = "SineModsCategory";
        createSineGroup(title);
        title.innerHTML = `<h1>${this.versionBrand} Mods</h1>`;
        document.querySelector("#mainPrefPane").insertBefore(title, document.querySelector(`[data-category="paneGeneral"]`));

        // Create group.
        const newGroup = document.createElement("groupbox");
        newGroup.id = "sineInstallationGroup";
        newGroup.className = "highlighting-group";
        createSineGroup(newGroup);
        ...
        newSettingsButton.innerHTML = this.settingsSVG;
        ...
        document.querySelector("#mainPrefPane").insertBefore(newGroup, document.querySelector(`[data-category="paneGeneral"]`));

        const installedGroup = document.createElement("groupbox");
        installedGroup.id = "sineInstalledGroup";
        installedGroup.className = "highlighting-group";
        createSineGroup(installedGroup);

        const groupHeader = document.createElement("hbox");
        groupHeader.id = "sineInstalledHeader";
        groupHeader.innerHTML = "<h2>Installed Mods</h2>";

        let modsDisabled = UC_API.Prefs.get("sine.mods.disable-all").value;
        const groupToggle = document.createElement("moz-toggle");
        groupToggle.className = "sinePreferenceToggle";
        groupToggle.title = `${modsDisabled ? "Enable" : "Disable"} all mods`;
        if (!modsDisabled) groupToggle.setAttribute("pressed", "true");
        // Logic to disable mod.
        ...
        installedGroup.appendChild(groupHeader);

        const groupDesc = document.createElement("description");
        groupDesc.className = "description-deemphasized";
        groupDesc.textContent = `${this.versionBrand} Mods you have installed are listed here.`;
        installedGroup.appendChild(groupDesc);

        const indent = document.createElement("hbox");
        ...
        modsList.id = "sineModsList";
        installedGroup.appendChild(modsList);
        
        document.querySelector("#mainPrefPane").insertBefore(installedGroup, document.querySelector(`[data-category="paneGeneral"]`));
    },
